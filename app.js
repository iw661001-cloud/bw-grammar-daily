...
// 題庫會陸續擴充題量，不能把分母寫死成10，要讀題庫json的實際長度。
async function trackSummary(track) {
  const [itemsSnap, questions] = await Promise.all([
    db.collection("self_grammar").doc(track.id).collection("items").get(),
    fetch(track.file).then((res) => res.json()),
  ]);
  let attempted = 0;
  let needsReview = 0;
  ...
  return { attempted, needsReview, total: questions.length };
}
...
function renderTrackCard(track, s) {
  ...
  return `
    <a class="track-card" href="track.html?t=${track.id}">
      ...
      <div class="track-stats">已作答 ${s.attempted}/${s.total} 題${reviewText}</div>
    </a>
  `;
}
