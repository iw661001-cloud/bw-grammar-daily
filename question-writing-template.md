...
|---|---|---|---|---|---|
| senior-grammar | 高中/學測文法 | mc（句子結構/分類辨識題） | 高中/學測 | 6題（句子結構子句判斷×2、時態判斷×2、假設語氣判斷×2） | sg-26 開始 |
| toeic-grammar | 多益 Part 5 | mc（句子結構/分類辨識題） | 多益 | 9題（句子結構子句判斷×3、時態判斷×3、假設語氣判斷×3） | tg-26 開始 |

**這次額外要求**：
- 這是全新題型（句子結構／分類辨識題），第一次委託，**先出這15題確認品質穩定，不要預設之後一定會擴大**
- 格式、3種出題角度、深化變化、分類歸屬規則，見「二、JSON格式規範」的「句子結構／分類辨識題」小節，務必先讀那段再開始寫
- 「句子結構（子句判斷）」的`category`固定填`"句子結構"`；「時態判斷」「假設語氣判斷」的`category`分別沿用既有的`"時態"`／`"假設語氣"`，**不要自己開新分類**
- `options`是句子片段時，建議每個片段不超過25個字，避免手機小螢幕排版問題
- 3種角度可以混用深化變化（問功能/問線索詞/問實際事實）跟基本版（問位置/問時態名稱/問假設類型），不用每題都用深化版，整批盡量兩種都出現，增加變化

**國中單字擴充（jv-36開始）暫緩，先完成上面這批句子結構題型**：`junior-vocab-batch2-words.txt`的150個字尚未出完（jv-36~jv-85已上線，words 51-200還沒委託），下次要繼續時再另外開任務欄位。
