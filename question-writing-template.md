...
|---|---|---|---|---|---|
| senior-vocab | 高中/學測單字 | mc | 高中/學測 | 50題（目標字見下方清單） | sv-186 開始 |

**這次額外要求**：
- 目標字清單：`D:\Users\bw5418\projects\english-study\00-自學\_drafts\senior-vocab-priority-batch4.json`，欄位說明同前幾批（`level`/`pos`/`zh`僅供參考定位，不可照抄當解析內容）
- 沿用規則：GSAT真題頻率排序選字，例句/選項/解析全部自己原創；格式照「二、JSON格式規範」開頭的schema逐字對照（上一批已修正過格式問題，維持這個標準）
