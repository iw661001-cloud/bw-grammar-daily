...
- **回報保持簡短，不需要詳細自評報告**（不用列「格式正確✓」「分布均勻✓」這類清單）：Claude Code 每次都會自己重新讀檔案、跑檢查腳本獨立驗證，不會採信自評內容，寫長篇報告只是徒增使用者要複製貼上的文字量
...
```json
{
  ...
  "wordGroup": "（選填）只有在**刻意**用不同例句重複練習同一個字時才加這個欄位，值填該字的基本形（例如「accompany」），同一組所有題目都填一樣的值。預設情況下同一題庫不重複出目標字，這個欄位不是用來補救不小心重複的字，是使用者明確決定要加強某個字才用"
}
```

**出題核心原則**：四個選項要「看起來都合理」，最好是字形相似、詞性相同、或意思相近容易混淆的字，不要出現一眼就能刪掉的無關選項（例如用「香蕉」當文法題的錯誤選項）。這樣才能真正測出理解程度，不是用消去法猜答案。

**格式必須逐字照抄上面的schema，不要自己發揮變體**（2026-09-04發生過的實際錯誤，之後不要再犯）：
...
- 正確答案的位置（`answerIndex`）交回前自己算一次整批的0~3分布次數，確認沒有明顯集中在少數幾個數字（規則5已經有講，這裡再次強調因為2026-09-04這批集中在B/C選項）
...
|---|---|---|---|---|---|
| senior-vocab | 高中/學測單字 | mc | 高中/學測 | 50題（目標字見下方清單） | sv-236 開始 |

**這次額外要求**：
- 目標字清單：`D:\Users\bw5418\projects\english-study\00-自學\_drafts\senior-vocab-priority-batch5.json`，欄位說明同前幾批（`level`/`pos`/`zh`僅供參考定位，不可照抄當解析內容）
- 沿用規則：GSAT真題頻率排序選字，例句/選項/解析全部自己原創；格式照「二、JSON格式規範」開頭的schema逐字對照
